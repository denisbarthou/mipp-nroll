...
extern "C" {
double bp_15(uint32_t *, TYPE *, TYPE *, TYPE *,double *);
}

double bp_15(uint32_t *H, TYPE *VN, TYPE *M, TYPE *C, double *event) {
    ...
    for (int c=0; c<BLOCKI ; c+=15) {
       ...
       auto sign11 = mipp::Msk<nv>(false);
       mipp::Reg<TYPE> min111= mipp::Reg<TYPE>(max);
       mipp::Reg<TYPE> min211 = mipp::Reg<TYPE>(max);
       mipp::Reg<TYPE> cval11, cabs11, cres11;
       auto sign12 = mipp::Msk<nv>(false);
       mipp::Reg<TYPE> min112= mipp::Reg<TYPE>(max);
       mipp::Reg<TYPE> min212 = mipp::Reg<TYPE>(max);
       mipp::Reg<TYPE> cval12, cabs12, cres12;
       auto sign13 = mipp::Msk<nv>(false);
       mipp::Reg<TYPE> min113= mipp::Reg<TYPE>(max);
       mipp::Reg<TYPE> min213 = mipp::Reg<TYPE>(max);
       mipp::Reg<TYPE> cval13, cabs13, cres13;
       auto sign14 = mipp::Msk<nv>(false);
       mipp::Reg<TYPE> min114= mipp::Reg<TYPE>(max);
       mipp::Reg<TYPE> min214 = mipp::Reg<TYPE>(max);
       mipp::Reg<TYPE> cval14, cabs14, cres14;
       ...
       vC[v]= vVN[H[(c+ 0)*BLOCKJ+v]] - vM[(c + 0)*BLOCKJ+v];
         ...
         vC[v]= vVN[H[(c+ 11)*BLOCKJ+v]] - vM[(c + 11)*BLOCKJ+v];
         cabs11 = mipp::abs(vC[v]);
         cres11 = min111;
         sign11 ^= mipp::sign(vC[v]);
         min111 = mipp::min(min111, cabs11);
         min211 = mipp::min(min211,mipp::max(cabs11,cres11));
         vC[v]= vVN[H[(c+ 12)*BLOCKJ+v]] - vM[(c + 12)*BLOCKJ+v];
         cabs12 = mipp::abs(vC[v]);
         cres12 = min112;
         sign12 ^= mipp::sign(vC[v]);
         min112 = mipp::min(min112, cabs12);
         min212 = mipp::min(min212,mipp::max(cabs12,cres12));
         vC[v]= vVN[H[(c+ 13)*BLOCKJ+v]] - vM[(c + 13)*BLOCKJ+v];
         cabs13 = mipp::abs(vC[v]);
         cres13 = min113;
         sign13 ^= mipp::sign(vC[v]);
         min113 = mipp::min(min113, cabs13);
         min213 = mipp::min(min213,mipp::max(cabs13,cres13));
         vC[v]= vVN[H[(c+ 14)*BLOCKJ+v]] - vM[(c + 14)*BLOCKJ+v];
         cabs14 = mipp::abs(vC[v]);
         cres14 = min114;
         sign14 ^= mipp::sign(vC[v]);
         min114 = mipp::min(min114, cabs14);
         min214 = mipp::min(min214,mipp::max(cabs14,cres14));
         }
      auto cst10 = mipp::blend(zero, min20 , zero>min20);
       ...
       auto cst111 = mipp::blend(zero, min211 , zero>min211);
       auto cst211 = mipp::blend(zero, min111, zero>min111) ;
       auto cst112 = mipp::blend(zero, min212 , zero>min212);
       auto cst212 = mipp::blend(zero, min112, zero>min112) ;
       auto cst113 = mipp::blend(zero, min213 , zero>min213);
       auto cst213 = mipp::blend(zero, min113, zero>min113) ;
       auto cst114 = mipp::blend(zero, min214 , zero>min214);
       auto cst214 = mipp::blend(zero, min114, zero>min114) ;
      ...
      cval11=vC[v];
           cabs11 = mipp::abs(cval11);
           cres11 = mipp::blend(cst111,cst211, cabs11==min111);
           auto csig11= sign11 ^ mipp::sign(cval11);
           cres11 = mipp::copysign(cres11,csig11);
           vM[(c + 11)*BLOCKJ+v] = cres11;
           vVN[H[(c+ 11 )*BLOCKJ+v]] = vC[v] + cres11;
      cval12=vC[v];
           cabs12 = mipp::abs(cval12);
           cres12 = mipp::blend(cst112,cst212, cabs12==min112);
           auto csig12= sign12 ^ mipp::sign(cval12);
           cres12 = mipp::copysign(cres12,csig12);
           vM[(c + 12)*BLOCKJ+v] = cres12;
           vVN[H[(c+ 12 )*BLOCKJ+v]] = vC[v] + cres12;
      cval13=vC[v];
           cabs13 = mipp::abs(cval13);
           cres13 = mipp::blend(cst113,cst213, cabs13==min113);
           auto csig13= sign13 ^ mipp::sign(cval13);
           cres13 = mipp::copysign(cres13,csig13);
           vM[(c + 13)*BLOCKJ+v] = cres13;
           vVN[H[(c+ 13 )*BLOCKJ+v]] = vC[v] + cres13;
      cval14=vC[v];
           cabs14 = mipp::abs(cval14);
           cres14 = mipp::blend(cst114,cst214, cabs14==min114);
           auto csig14= sign14 ^ mipp::sign(cval14);
           cres14 = mipp::copysign(cres14,csig14);
           vM[(c + 14)*BLOCKJ+v] = cres14;
           vVN[H[(c+ 14 )*BLOCKJ+v]] = vC[v] + cres14;
      }
       }} 
